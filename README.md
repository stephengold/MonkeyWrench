...
Before applying the preliminary transformation, the texture filename
is URL encoded.
This includes converting each blank to "%20".

The preliminary transformation is either:
+ `NoOp` which leaves the filename unchanged or
...
using filenames generated from the format strings in its search path.
...
1. "source/UV_Barn_Assets.psd" from the first format string "%s%s%s",
   which is equivalent to "%1$s%2$s%3$s"
2. "source/textures/UV_Barn_Assets.JPG"
   from the 2nd format string "%stextures/%s.JPG"
3. "Textures/UV_Barn_Assets.psd"
   from the 3rd format string "Textures/%2$s%3$s"
